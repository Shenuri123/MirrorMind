...
class TwitterState extends State<Twitter> {
  late Stream<List<double>> weeklyTwitterDataStream;

  @override
  void initState() {
    super.initState();
    weeklyTwitterDataStream = getTwitterDataAsStream();
  }

  Stream<List<double>> getTwitterDataAsStream() async* {
  ...
  Widget build(BuildContext context) {
    return AspectRatio(
      ...
      child: StreamBuilder<List<double>>(
        stream: weeklyTwitterDataStream,
        builder: (context, snapshot) {
          print("Received a new snapshot: ${snapshot.data}");
          if (snapshot.hasData) {
          ...
          return Center(
              child:
                  CircularProgressIndicator()); // Loading indicator until data is fetched.
        },
      ),
    );
  }}
