...
class DepressionAnalysis extends StatefulWidget {
  @override
  _SocialMediaGraphState createState() => _SocialMediaGraphState();
}

class _SocialMediaGraphState extends State<DepressionAnalysis> {
  ...
  void getDataFromFirebase() async {
    ...
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        // depressionData.add(DepressionData(timestamp.toString(), result));
      }
  ...
  void shareReport() async {
    ...
    final filePath = '${directory!.path}/Quiz Game Report.pdf';

    final file = File(filePath);
    if (await file.exists()) {
      Share.shareFiles([filePath], text: 'Quiz Game Report');
    } else {
  ...
  Future<void> generateSocialMediaPDF() async {
    ...
    if (status.isGranted) {
      final directory = await getExternalStorageDirectory();
      if (directory != null) {
        final file = File('${directory.path}/Quiz Game Report.pdf');
